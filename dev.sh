...
# Development script for community Chat (Leptos).
#
...
#
# Units:
#   server  cargo run --release -p community-server   (randomized port)
...
PG_PORT=""
PG_CONTAINER="community-pg-$REGISTRY_KEY"
...
select_pg_port() {
    ...
    if [ -z "$PG_PORT" ] || ! port_available "$PG_PORT"; then
        ...
        for i in $(seq 1 "$MAX_PORT_ATTEMPTS"); do
            p="$(random_even_port)"
            if [ "$p" != "$SERVER_PORT" ] && [ "$p" != "$WEB_PORT" ] && port_available "$p"; then
                PG_PORT="$p"; break
            fi
        done
        [ -z "$PG_PORT" ] && { echo -e "${RED}Error: could not find a free port for Postgres${NC}" >&2; exit 1; }
    fi
    # Persist alongside the app ports (keep SERVER_PORT/WEB_PORT, refresh PG_PORT).
...
do_start() {
    ...
    (
        ...
        cleanup() {
            ...
            # NOTE: we deliberately do NOT `pkill -f "community-server"`
        ...
        (
            ...
            if cargo build --release -p community-server; then
                ...
                bin="$SCRIPT_DIR/target/release/community-server"
                if [ -x "$bin" ]; then
                    exec "$bin"
                else
                    exec cargo run --release -p community-server
                fi
            else
        ...
        # proxies /ws (websocket) plus the two session mints — /auth/session
        # (OIDC) and /auth/guest (read-only), both plain HTTP POSTs — to the
        # backend; /auth/callback stays with trunk, it is the SPA landing page
        # that completes the ceremony client-side. In production the server
        # serves SPA and API on one origin, so this table exists only here;
        # a mint the client calls but this table misses answers 405 from
        # trunk, not the backend.
        # Three proxies exceed what trunk's CLI flags express (--proxy-backend
        # is single-use), so we render Trunk.dev.toml — the committed config
        # plus [[proxy]] sections carrying the live port — and serve with
        # --config.
        echo "[web] Starting trunk on port $WEB_PORT (proxy /ws,/auth -> $SERVER_PORT)..."
        (
            cd "$LEPTOS_PATH"
            f="$sf.web.status"
            ...
            {
                cat Trunk.toml
                printf '\n# Generated by dev.sh (gitignored): proxies with the live backend port.\n'
                printf '[[proxy]]\nbackend = "ws://127.0.0.1:%s/ws"\nws = true\n\n' "$SERVER_PORT"
                printf '[[proxy]]\nbackend = "http://127.0.0.1:%s/auth/session"\n\n' "$SERVER_PORT"
                printf '[[proxy]]\nbackend = "http://127.0.0.1:%s/auth/guest"\n' "$SERVER_PORT"
            } > Trunk.dev.toml
            exec trunk serve \
...
cmd_start() {
    ...
    echo ""
    echo -e "${BOLD}Starting community dev environment (Leptos)${NC}"
    echo ""
    select_ports
    select_pg_port
    if [ "$STORAGE_ENGINE" = "postgres" ]; then
        echo -e "  ${DIM}Ports:${NC} server=$SERVER_PORT  web=$WEB_PORT  postgres=$PG_PORT  ${DIM}(randomized; sticky in .dev-ports)${NC}"
    else
...
cmd_stop() {
    ...
    local pid
    pid="$(cat "$PID_FILE")"
    echo -e "Stopping community dev environment (PID $pid)..."
