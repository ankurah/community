...
do_start() {
    ...
    (
        ...
        # proxies /ws (websocket) and /auth/session (the OIDC session mint, a
        # plain HTTP POST) to the backend; /auth/callback stays with trunk, it
        # is the SPA landing page that completes the ceremony client-side.
        # Two proxies exceed what trunk's CLI flags express (--proxy-backend is
        # single-use), so we render Trunk.dev.toml — the committed config plus
        # [[proxy]] sections carrying the live port — and serve with --config.
